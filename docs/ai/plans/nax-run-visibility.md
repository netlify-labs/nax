...
## 10. Implementation notes (2026-07-22, branch feat/run-visibility)

- 4.C deviation: `usageTotals` is computed directly via
  `usageSummariesForRunState(runState).total` instead of reading
  `artifacts/usage.json` — the artifact is generated by that same function
  over the same run records, so reading it back adds I/O without new data.
- The real `UsageSummary` vocabulary is `totalCreditsCost` / `totalTokens` /
  `stepsCount` / `creditLimitExceeded` (src/types.js:18-26). The
  credits/inputTokens/costUsd shape quoted in §4.C came from the mock-fixture
  bead's *proposed* schema, not live data; contracts and displays use the
  real fields.
- `nax costs` description avoids the word "recent" — the CLI surface tests
  guard root help against /recent/ (legacy `nax recent` removal).
