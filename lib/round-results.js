...
function pickAgentReplyComments(comments, { all = false } = {}) {
...
function fetchRoundResults({
  ...
  return issueNumbers.map((issueNumber, index) => {
    ...
    const replies = pickAgentReplyComments(issue.comments || [], { all: embedAll })
...
function findRunnerResultAfter(comments, startIndex) {
  ...
  for (let i = startIndex + 1; i < comments.length; i += 1) {
    ...
    if (RUNNER_RESULT_MARKER_PATTERN.test(body)) return comments[i]
  }
  return null
}
