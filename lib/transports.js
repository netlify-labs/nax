...
function detectTransports({ projectRoot = process.cwd(), env = process.env } = {}) {
  ...
  return [
    ...
    },
    {
      ...
      title: 'Via Netlify API locally on this machine',
...
function formatTransportSetupHelp(detections) {
  ...
  const local = byId.get(NETLIFY_API_TRANSPORT) || byId.get('local')
  ...
  lines.push('  3. Re-run nax and choose "Via Netlify API locally on this machine".')
