...
const { formatUsageSummary, usageSummariesForRunState } = require('./agent-run-results')
...
function runsArtifactsDir(runState, step) {
  return path.join(stepArtifactsDir(runState, step), 'runs')
}
...
function attemptsForAgent(runsDir, agent) {
  ...
  return existingAttemptFiles(runsDir, safeAgent).map((attempt) => {
    const metadataPath = `runs/${safeAgent}.attempt-${attempt.number}.json`
    const resultPath = `runs/${safeAgent}.attempt-${attempt.number}.md`
  ...
  return {
    ...
    runs: (step.runs || []).map((run) => {
      ...
      return {
        ...
        runnerId: run.runnerId || '',
        sessionId: run.sessionId || '',
        resultPath: `runs/${agent}.md`,
        metadataPath: `runs/${agent}.json`,
...
function buildStepMarkdown({ runState, step }) {
...
function buildTopSummaryMarkdown(runState = {}) {
  ...
  if (steps.length > 0) {
    ...
    steps.forEach((step, index) => {
      lines.push(`${index + 1}. [${step.title || step.id}](#${safeArtifactName(step.title || step.id)})`)
    })
  }
  for (const step of steps) {
    lines.push('', '---', '')
    lines.push(readFileIfExists(path.join(stepArtifactsDir(runState, step), 'summary.md')).trim() || `## ${step.title || step.id}`)
  }
  lines.push('')
  return lines.join('\n')
}
