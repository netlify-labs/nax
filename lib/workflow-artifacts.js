...
function runArtifactLinks(runState, step, run, linkPrefix = '') {
  ...
  if (fs.existsSync(latestMarkdown)) links.push(`[result](${joinLinkPath(linkPrefix, 'runs', `${agent}.md`)})`)
  if (fs.existsSync(latestJson)) links.push(`[metadata](${joinLinkPath(linkPrefix, 'runs', `${agent}.json`)})`)
