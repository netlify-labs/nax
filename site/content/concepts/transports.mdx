...
When every selected run uses Auto model and effort, transport selection keeps
its normal behavior. If any run pins a model or effort, Auto selects
`netlify-api`. Explicit GitHub transport rejects the configuration before
dispatch because the pinned Action has a provider channel but no real model or
effort channel. NAX never drops or prompt-encodes those settings.
