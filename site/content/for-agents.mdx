...
Override the agent providers, then optionally configure their real models:
...
Provider lists belong only in `--agents` and `--step-agents`. Model and effort
flags require explicit provider mappings and can be repeated. `auto` clears a
pin and becomes omission on the wire. Unknown future IDs pass through with a
warning; known incompatible pairs fail before mutation. Pinned configuration
requires the Netlify API transport.
