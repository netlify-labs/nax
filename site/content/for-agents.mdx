...
OpenCode model IDs are provider-scoped (for example `moonshotai/kimi-k3`,
`z-ai/glm-5.2`, `deepseek/deepseek-v4-pro`) and some expose only High/Max, where
Max is sent as `xhigh`. See the [model and effort catalog](/reference/workflow-files#current-model-and-effort-catalog)
