...
```sh
nax run --resume <run-id> --dry
nax run --resume <run-id>
...
Interrupted `netlify-api` runs write state under `.nax/workflows/<run-id>/workflow.json`. Starting `nax run` again offers to resume unfinished work. `--resume` keeps finished agents and resubmits only unfinished ones; see [Resume runs](/guides/resume-runs).
