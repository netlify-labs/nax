...
NAX exposes 14 entity-first tools. Names use `noun_action`, so related operations sort together and remain predictable for agents.
...
| `run_resume` | Resume a failed or interrupted run in place. | Yes; idempotent |
...
Requires `run_id` and one `view`: `summary`, `details`, `graph`, `events`, or `findings`.

- `details` also returns a `findings` summary (count, severity histogram, top ten ranked findings) when the run's workflow declares findings.
- `findings` returns the full `findings.json` artifact, also available as the resource `nax://scopes/{scope_id}/runs/{run_id}/findings`.
...
Requires `run_id` and a stable `request_id`; `include_cancelled` also resubmits cancelled agents. It resumes a failed or interrupted Netlify API run in place: completed agents are kept, running agents are polled, and only failed or never-sent agents are resubmitted with their saved prompts. The result includes the per-agent `keep`/`poll`/`resubmit`/`skip` preview. Identical requests replay the first result and never start a second resume.

It refuses before starting anything with a recoverable code: `run_locked` (another process is executing the run; use `run_wait`), `flow_changed_since_run` or `resume_plan_unavailable` (plan a new run), `resume_ambiguous_submission` or `branch_moved_since_run` (a human must decide; the CLI's `nax run --resume <run-id> --force` overrides), `resume_auth_failure`, or `not_resumable`. See [Resume runs](/guides/resume-runs).

```json
{
  "run_id": "run_01JEXAMPLE",
  "request_id": "request_resume_01JEXAMPLE"
}
```
...
| `nax://scopes/{scope_id}/runs/{run_id}/findings` | Structured findings (`findings.json`) when the run's workflow declares findings. |
