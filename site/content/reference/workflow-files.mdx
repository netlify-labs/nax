...
| `findings` | no | `{ step, adapter }` naming the agent step whose result holds structured findings and the adapter that parses it (for example `review-consensus`). See [Hand off findings](/guides/hand-off-findings). |
