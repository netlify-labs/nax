...
request-marker-decorated UTF-8 prompt. It tries inline delivery, then
deterministic compaction, then a site-scoped Netlify Blob. The SDK returns
`prompt-too-large` only when none of those paths fits. Confirm that the selected
site is correct and that `NETLIFY_AUTH_TOKEN` can access it before increasing
the budget.
...
run retains its blob for diagnosis and safe retry until the reference expires.
