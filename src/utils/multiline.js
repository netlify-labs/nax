...
function rowsBelowCursor({ lines, lineIdx, colIdx }, columns) {
  ...
  const wrapsFor = (len) => Math.max(1, Math.ceil(len / cols) || 1)
  const currentWraps = wrapsFor(lines[lineIdx].length)
  const currentWrapRow = Math.min(currentWraps - 1, Math.floor(colIdx / cols))
  let rows = Math.max(0, currentWraps - 1 - currentWrapRow)
  for (let i = lineIdx + 1; i < lines.length; i += 1) {
    rows += wrapsFor(lines[i].length)
  }
  return rows
}
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (ch === '\r' || ch === '\n') {
          ...
          if (isPasting) {
            ...
            const rowsBelow = rowsBelowCursor({ lines, lineIdx, colIdx }, process.stdout.columns)
            if (rowsBelow > 0) process.stdout.write(`\x1b[${rowsBelow}B`)
            process.stdout.write('\r\n')
            cleanup()
            resolve(collapseTrailingNewlines(lines.join('\n')))
            return
          }
          continue
        }
...
module.exports = { collapseTrailingNewlines, multiline, moveCursor, rowsBelowCursor }
