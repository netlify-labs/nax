...
function stepWithAttempts() {
  return {
    ...
    attempts: [{ attemptId: 'a1', agent: 'codex', instanceId: 'codex:auto:auto', status: 'completed', runnerId: 'r1', resultText: STALE, resultTextPath: '' }],
  }
}

test('prompt chaining reads only current attempts', () => {
  const completed = new Map([['review', stepWithAttempts()]])
...
test('findings are built from current attempts only', () => {
  const runState = { runId: 'contract-run', flowId: 'review', steps: [stepWithAttempts()] }
