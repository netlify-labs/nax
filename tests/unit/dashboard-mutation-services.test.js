...
test('mutation service retries a terminal agent in an active step', async () => {
  ...
  assert.match(String(durable.steps[0].runs[1].attemptId), /^[0-9a-f-]{36}$/)
  const archived = /** @type {Array<{ runnerId: string, status: string }>} */ (durable.steps[0].attempts || [])
  assert.deepEqual(archived.map((attempt) => [attempt.runnerId, attempt.status]), [['runner-old', 'completed']])
  const saved = JSON.parse(fs.readFileSync(path.join(durable.dir, 'workflow.json'), 'utf8'))
  assert.deepEqual(saved.steps[0].attempts.map((/** @type {{ runnerId: string }} */ attempt) => attempt.runnerId), ['runner-old'])
})
