...
import { ActionIcon, Alert, Badge, Box, Button, Checkbox, Code, Divider, Group, Modal, Paper, ScrollArea, SegmentedControl, Select, Spoiler, Stack, Text, Textarea, Tooltip } from '@mantine/core'
import { notifications } from '@mantine/notifications'
import { CheckCircle2, ExternalLink, FileSearch, Play, RefreshCw } from 'lucide-react'
    ...
    >
      <Stack gap="md">
        ...
        <Paper className="run-followup-panel" withBorder>
          <Stack gap="sm">
            <SegmentedControl
              ...
              onChange={changeMode}
              data={[
                { value: 'follow-up-thread', label: 'Follow up on existing thread', disabled: threadTargets.length === 0 },
        ...
        <Paper className="run-followup-plan" withBorder>
          <Stack gap={4}>
            ...
            {models.length > 0 && target ? models.map((model) => (
              <Text key={model} size="sm">{followupPlanLine(model, mode, target)}</Text>
            )) : (
              <Text size="sm" c="dimmed">Select a target and at least one model.</Text>
            )}
            {mode === 'follow-up-thread' && target?.runnerId ? (
              <Text size="xs" c="dimmed">Existing runner: <Code>{target.runnerId}</Code></Text>
            ) : null}
          </Stack>
        </Paper>
        ...
        <Group justify="space-between" align="center">
          ...
          <Button variant="subtle" color="gray" onClick={onClose} disabled={submitting}>Close</Button>
        </Group>
      </Stack>
    </Modal>
  )
